...
public class EventSendStudentEmailsBackend implements GwtRpcImplementation<SendStudentEmailsRpcRequest, GwtRpcResponseNull> {
	...
	public void sendReminderEmail(Event event, long studentId, String nameFormat,BaseSession baseSession) throws Exception {
		Student student = StudentDAO.getInstance().get(studentId);
		Email email = Email.createEmail();
		if (student != null && student.getEmail() != null && !student.getEmail().isEmpty()) {
			email.addRecipientBCC(student.getEmail(), student.getName(nameFormat));
			email.setSubject("Reminder: " + event.getEventName() + " is starting soon");
			email.setText("This is a reminder that the event " + event.getEventName()+ " is starting at "+baseSession.getEventBeginDate()+". Please make sure to attend on time.");
			email.send();
		}
	}
}
